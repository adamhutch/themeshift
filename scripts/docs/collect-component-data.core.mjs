...
function createRuntimePaths(rootDir) {
  return {
    rootDir,
    componentsDir: path.join(rootDir, 'packages/ui/src/components'),
    outputPath: path.join(
      rootDir,
      'apps/ui-app/src/apiReference/generated/components.ts'
    ),
    packageJsonPath: path.join(rootDir, 'packages/ui/package.json'),
    tsConfigPath: path.join(rootDir, 'packages/ui/tsconfig.build.json'),
  };
}

let runtimePaths = createRuntimePaths(defaultRootDir);

function setRuntimePaths(rootDir) {
  runtimePaths = createRuntimePaths(rootDir);
}
...
async function hasIndexFile(componentName) {
  const candidatePaths = ['index.tsx', 'index.ts'].map((fileName) =>
    path.join(runtimePaths.componentsDir, componentName, fileName)
  );
...
async function getComponentNames() {
  const packageJson = JSON.parse(
    await readFile(runtimePaths.packageJsonPath, 'utf8')
  );
  const packageExports = new Set(Object.keys(packageJson.exports ?? {}));
  const entries = await readdir(runtimePaths.componentsDir, {
    withFileTypes: true,
  });
...
function createProgram() {
  ...
  const parsedConfig = ts.parseJsonConfigFileContent(
    configFile.config,
    ts.sys,
    path.dirname(runtimePaths.tsConfigPath)
  );
  ...
  const sourceFiles = program
    ...
    .filter((sourceFile) =>
      normalizePath(sourceFile.fileName).startsWith(
        normalizePath(runtimePaths.componentsDir)
      )
    );
  ...
  return {
    collectApiReference(componentName) {
      const componentSourceFiles = sourceFiles.filter((sourceFile) =>
        normalizePath(sourceFile.fileName).startsWith(
          normalizePath(path.join(runtimePaths.componentsDir, componentName))
        )
      );
...
function getLocalTypeDeclarations(componentName, sourceFiles) {
  const componentTypesSourceFile = sourceFiles.find(
    (sourceFile) =>
      ...
      normalizePath(
        path.join(runtimePaths.componentsDir, componentName, 'types.ts')
      )
  );
...
function getArrowFunctionInitializer(initializer) {
  ...
  if (ts.isCallExpression(initializer)) {
    ...
    if (firstArgument) {
      const nestedArrowFunction = getArrowFunctionInitializer(firstArgument);

      if (nestedArrowFunction) {
...
export async function runCollectComponentData({
  log = true,
  rootDir = defaultRootDir,
  syncBadges = true,
  writeOutput = true,
} = {}) {
  setRuntimePaths(rootDir);
  ...
  const output = await createOutput(componentData);

  if (writeOutput) {
    await mkdir(path.dirname(runtimePaths.outputPath), { recursive: true });
    await writeFile(runtimePaths.outputPath, output);
  }

  if (syncBadges) {
    await syncUiComponentBadges({ rootDir: runtimePaths.rootDir });
  }

  if (log) {
    console.log(
      `Collected component data for ${componentData.length} components`
    );
  }
